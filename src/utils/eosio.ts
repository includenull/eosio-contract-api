import { ABI, Bytes, Name, Serializer, UInt64 } from '@wharfkit/antelope';
...
export function serializeEosioName(name: string): string {
    const v = Name.from(name).value;

    return serializeUInt(BigInt(v.value.toString())).toString();
}
...
export function deserializeEosioName(data: string): string {
    ...
    return Name.from(UInt64.from(bytes)).toString();
}
...
export function splitEosioToken(
    asset: string,
    contract?: string
): { amount: string; token_symbol: string; token_precision: number; token_contract?: string } {
...
export function deserializeEosioType(type: string, data: Uint8Array | string, abi: ABI, _checkLength = true): any {
    ...
    for (const transaction of data) {
        if (transaction[0] === 'transaction_trace_v0') {
            ...
            transactions.push({
                ...
                traces: transaction[1].action_traces
                    .map(trace => {
